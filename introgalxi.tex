...
\be
w(\theta) = 0.36 \left( \frac{1'}{\theta} \right)^{0.8} \, \frac{0.1}{\Delta z}.
\ee
As a sanity check, the slope is consistent with the latest DES measurements \citep{Elvin-Poole:2017xsf}, but the assumed amplitude is actually a bit smaller than the observations, so this might lead to an under-estimate of the signal to noise. We also choose 4 galaxies per square arcminute in a given redshift bin of width $\Delta z=0.2$. Most of the signal comes from pixels closest to the cluster center, so in this case including galaxies to only $2'$ is sufficient
...
The results of this estimate are that {\bf the expected signal to noise is large}. Even for the extremely low density redMaGic galaxies in DES, the signal to noise from a single tomographic bin with $0.6<z<0.75$, the expected signal to noise is 0.2. Therefore, stacking only 500 clusters (the Year 1 catalog has closer to 10,000) leads to a 10\% mass determination. With its more precise photometric redshifts and larger background galaxy density, {\bf LSST could measure the mass of a single cluster with 10\% uncertainty} using the \atf\ technique.
